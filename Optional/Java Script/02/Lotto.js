...
function generateNumbers() {
  ...
  */
  let numbers = []; //로또 번호들을 담을 빈 배열
  while (numbers.length < 6) { //숫자 6개를 뽑을 때까지 반복
    //Math.floor(Math.random() * 45) + 1 랜덤 정수를 1~45까지
    let randomNum = Math.floor(Math.random() * 45) + 1;
    //!numbers.includes(randomNum) randomNum에 같은 숫자가 업다면 푸쉬
...
function displayNumbers(numbers, winningNumbers = []) {
  ...
  */
  ...
  NumDiv.classList.add(`number-set`);
  ...
  numbers.forEach(function (randomNum) {
    ...
    if (winningNumbers.includes(randomNum)) {
      numEl.classList.add('match');
    }
    numEl.textContent = randomNum;
    NumDiv.appendChild(numEl);
  });
...
generateWinningNumbers.addEventListener('click', () => {
  ...
  */
  winningNumbers = generateNumbers(); // 당첨 번호 생성
  winningNumContainer.innerHTML = ''; // 이전 당첨 번호를 화면에서 제거
...
generateLottoNumbers.addEventListener('click', () => {
  ...
  */
  // 당첨 번호가 생성되지 않은 경우 경고 메시지 표시
  if (winningNumbers.length !== 0) {
    lottoNumContainer.innerHTML = ''; // 이전에 생성된 로또 번호를 화면에서 제거
    for (let i = 0; i < 5; i++) {
      // 5번 반복하여 로또 번호 생성
      const lottoNumbers = generateNumbers(); // 로또 번호 생성
